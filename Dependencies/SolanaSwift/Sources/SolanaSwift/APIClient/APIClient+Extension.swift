...
public extension SolanaAPIClient {
    ...
    func getMinimumBalanceForRentExemption(span: UInt64, commitment: String) async throws -> UInt64 {
        // devnet try await getMinimumBalanceForRentExemption(dataLength: span, commitment: "finalized")
        // mainnet try await getMinimumBalanceForRentExemption(dataLength: span, commitment: "recent")
        try await getMinimumBalanceForRentExemption(dataLength: span, commitment: commitment)
    }
    ...
    func observeSignatureStatus(signature: String) -> AsyncThrowingStream<PendingTransactionStatus, Error> {
