...
enum JSONRPCAPIClientError: Error {
    case timeout
}
...
public class JSONRPCAPIClient: SolanaAPIClient {
    ...
    public func getFeeForMessage(message: Message, commitment: Commitment? = nil) async throws -> UInt64 {
        let result: Rpc<UInt64> = try await get(
            method: "getFeeForMessage",
            params: [
                message.serialize().base64EncodedString(),
    ...
    public func observeSignatureStatus(signature: String, timeout: Int = 60,
                                       delay: Int = 2) -> AsyncThrowingStream<PendingTransactionStatus, Error>
    {
        AsyncThrowingStream { continuation in
            let monitor = TransactionMonitor(
                ...
                timedOutHandler: {
                    continuation.yield(with: .failure(JSONRPCAPIClientError.timeout))
                }
            )
